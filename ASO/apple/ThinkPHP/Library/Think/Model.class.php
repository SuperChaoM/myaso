...
class Model {
    ...
    protected $auto_page        =   false;    //自动分页
    /**
     * 架构函数
     * 取得DB类的实例对象 字段检查
     * @access public
    ...
    public function add($data='',$options=array(),$replace=false) {
        ...
        //以下模型自动完善create_day
        if (in_array(strtolower($this->name),['amount_stream','task'])) {
            $data['create_day'] = date('Ymd');
        }
    ...
    protected function _after_select(&$resultSet,$options) {
        ...
        if ($this->auto_page) {
            ...
            $count = intval(end(end($this->query(" select count(1) from ({$sql}) as a"))));
            $GLOBALS['__PAGE_COUNT__'] = $count;
        }
    }
