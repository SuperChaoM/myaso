...
<div class="row" style="padding-left: 10px;">
    <form class="form-inline" action="" method="get">
        <a href="<?php echo U('add');?>" class="btn btn-sm btn-success">新增工作计划模板</a>
        <label class="inline">模板名称:</label>
        <input type="text" name="tpl_name" class="form-control" value="<?php echo ($tpl_name); ?>">
        <button type="submit" class="btn btn-purple btn-sm">
            <span class="ace-icon fa fa-search icon-on-right bigger-110"></span>
            搜索
        </button>
        <input type="hidden" name="m" value="Qwadmin">
        <input type="hidden" name="c" value="WorkTpl">
...
<table class="table table-striped table-bordered table-hover">
    ...
    <tr>
        ...
        <th style="width: 60px;"  class="task">模板名称</th>
        <th style="width: 60px;"  class="task">操作</th>
    </tr>
    </thead>
    <tbody class="task">
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$d): $mod = ($i % 2 );++$i;?><tr>
            ...
            <td><?php echo ($d['tpl_name']); ?></td>
            <td>
                <a class="btn btn-sm btn-success " href="<?php echo U('edit',['tpl_id'=>$d['tpl_id']]);?>">编辑</a>
                <a class="btn btn-sm btn-danger del_link"  href="javascript:;" val="<?php echo U('del',['tpl_id' => $d['tpl_id']]);?>">删除</a>
            </td>
