...
<div class="row" style="padding-left: 10px;">
    <form class="form-inline" action="" method="get">
        ...
        <td>
            <?php echo IForm::select(array('field' => 'status', 'options' => $status_name, 'remind' => '选择状态','width' => '120px'),$status);?>
        </td>
        <td>
            <label class="inline">创建时间:</label>
        </td>
        <td>
            <?php echo IForm::date(['field' => 'create_time_s'], $create_time_s);?>-<?php echo IForm::date(['field' => 'create_time_e'], $create_time_e);?>
        </td>
        <td>
        ...
        <input type="hidden" name="a" value="index">
            ...
            <td>
                <a  class="btn btn-success btn-sm" href="<?php echo U('import_file');?>">导入账号文件</a>
            </td>
            <td>
                <a  class="btn btn-danger btn-sm" href="<?php echo U('clear_email');?>">清空所有数据</a>
            </td>
        </tr>
        </table>
        <span class="label label-success" style="float: right">总结<?php echo ($GLOBALS['__PAGE_COUNT__']); ?>个账号</span>
    </form>
</div>
...
<table class="table table-striped table-bordered table-hover">
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$d): $mod = ($i % 2 );++$i;?><tr>
            ...
            <td style="text-align: left"><?php echo ($d['email']); ?></td>
            <td><?php echo ($d['password']); ?></td>
            <td>
            <?php if($d['status'] == 2): ?><span class="label label-warning">封禁</span><?php endif; ?>
            <?php if($d['status'] == 1): ?><span class="label label-success">正常</span><?php endif; ?>
            </td>
            <td><?php echo ($d['created_at']); ?></td>
            <td>
                <a class="btn btn-sm btn-danger del_link"  href="javascript:;" val="<?php echo U('del',['id' => $d['id']]);?>">删除</a>
            </td>
