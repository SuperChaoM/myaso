...
- (void)viewDidAppear:(BOOL)arg1 {
  ...
  //login Failed
	...
	if ([alertTitle isEqualToString:@"出错了"] && [alertMessage containsString:@"似乎有什么地方出问题"]){
		for (id act in alertActions){
			NSString *actTitle = [act title];
			if ([@"好" isEqualToString: actTitle]){
				NSLog(@"#cargo clicked 出错了-好");
				[self _actionSelected:act];
				break;
			}
		}
		[[StoreTaskManager sharedManager] appStore_markTaskFail:RET_CODE_ERR_PURCHASE];
		return;
	}
	...
	// 		[self _actionSelected:alertActions[0]];
